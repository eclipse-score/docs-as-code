...
def test_single_entry_json_no_path():
    ...
    assert result == [ExternalNeedsSource(bazel_module="repo", path_to_target="", target="needs_json")]

def test_multiple_entries():
    result = parse_external_needs_sources_from_DATA('["@repo1//:needs_json", "@repo2//:needs_json"]')
    assert result == [
        ExternalNeedsSource(bazel_module="repo1", path_to_target="", target="needs_json"),
        ExternalNeedsSource(bazel_module="repo2", path_to_target="", target="needs_json")
    ]

def test_multiple_entries_2():
    result = parse_external_needs_sources_from_DATA('["@repo1//:needs_json", "@repo2//path:needs_json"]')
