...
# target platform and fail Python toolchain resolution. Instead of `target_compatible_with` (which
# would only make `bazel build //...` skip them), each is wrapped in a rule whose incoming transition
# pins `//command_line_option:platforms` to `--host_platform`, so they always build and run in the
# host config regardless of the top-level `--platforms`.

def _to_host_platform_impl(settings, _attr):
    return {
...
_to_host_platform = transition(
    implementation = _to_host_platform_impl,
    inputs = ["//command_line_option:host_platform"],
    outputs = ["//command_line_option:platforms"],
)

def _host_binary_impl(ctx):
    # `actual` is a singleton list because of the outgoing transition.
    target = ctx.attr.actual[0]
    default_info = target[DefaultInfo]

    # Re-expose the wrapped binary under this target's name by symlinking its executable and
    # forwarding its runfiles, so `bazel run` keeps working.
    ...
    runfiles = ctx.runfiles([executable]).merge(default_info.default_runfiles)
    ...
    # Forward the wrapped binary's run environment (its `env` attr) so `bazel run`
    # of the wrapper still applies it.
    if RunEnvironmentInfo in target:
        providers.append(target[RunEnvironmentInfo])
    return providers

# Wrapper for runnable host tools (py_binary / py_venv / sphinx_build_binary).
_host_binary = rule(
    ...
    attrs = {
        "actual": attr.label(
            mandatory = True,
            doc = "The host-only target to build in the host configuration.",
            cfg = _to_host_platform,
        ),
...
def docs(source_dir = "docs", data = [], deps = [], scan_code = [], known_good = None, metamodel = None):
    ...
    _host_binary(
        ...
        actual = ":live_preview_combo_experimental_impl",
