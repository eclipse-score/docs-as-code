...
**\<check functions>** &mdash; `:check:`<br>
Check functions (comma separated) to be used for the Sphinx build. Only the
warnings generated by these check functions are considered. Because only one
`test_metadata` need is allowed per file, there is exactly one `:check:` per file.
If `:check:` is left empty, all checks are enabled.

**\<requirement id(s)>** &mdash; `:partially_verifies_list:` / `:fully_verifies_list:`<br>
